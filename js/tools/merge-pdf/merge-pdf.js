...
// Import the native sharing helper system (Relative path updated for subdirectory)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateUI() {
        ...
        // Reset Share button state when UI changes
    ...
    mergeBtn.addEventListener('click', async () => {
        ...
        mergeBtn.disabled = true;
        if (shareBtn) shareBtn.disabled = true; // Disable Share button during merge
        ...
        try {
            ...
            setTimeout(() => {
                ...
                // Enable Share Button on success
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', async () => {
            if (!mergedPdfBlob) return alert("Please merge PDFs first!");
            
            let name = fileNameInput.value.trim() || "sarixa-merged";
            if (!name.toLowerCase().endsWith(".pdf")) name += ".pdf";
            
            // Custom message for the merged file
            const customMessage = "Here is the PDF document I merged securely using *SArixa* (100% Private, Zero Uploads).";

            await sharePdfFile(mergedPdfBlob, name, shareBtn, customMessage);
        });
    }
});